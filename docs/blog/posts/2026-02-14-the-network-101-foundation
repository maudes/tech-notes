...
![OSI layers](tech-notes/blog/assets/OSI%20layers.png)
...
5. **Client layer 3:** Move on to the Network layer where the message is further divided into packets with an IP header which includes the source and destiniation IP. 
6. **Client layer 2:** In the data link layer, the message is again divided into frames with ethernet (or Wi-Fi) header and footer. This layer requires source and destination MAC addresses. Since the message usually needs to travel to the WAN (Internet) rather than staying on the local network (LAN), the client machine uses Address Resolution Protocl (ARP) to find the MAC address of the default gateway (the router).
...
9. **Router:** The router decapsulates the message up to layer 3 to inspect the desitnation IP address. If a match is found in its routing table, it forward the message. If not, it will route the message to the ISP Gateway. The above "routing behaviour" will have the router replaces the destination MAC address in the message. 
